...
func (d *Database) MakeTables() (err error) {
	...
	sqlStmt = `CREATE TABLE location_predictions (timestamp integer NOT NULL PRIMARY KEY, deviceid text, prediction TEXT, UNIQUE(timestamp));`
	_, err = d.db.Exec(sqlStmt)
	if err != nil {
		err = errors.Wrap(err, "MakeTables")
		logger.Log.Error(err)
...
func (d *Database) AddPrediction(timestamp int64, device string, aidata []models.LocationPrediction) (err error) {
	...
	stmt, err := tx.Prepare("insert or replace into location_predictions (timestamp,deviceid,prediction) values (?, ?, ?)")
	if err != nil {
		return errors.Wrap(err, "stmt AddPrediction")
	}
	defer stmt.Close()

	_, err = stmt.Exec(timestamp, deviceID, string(b))
	if err != nil {
...
func (d *Database) GetPrediction(timestamp int64, device string) (aidata []models.LocationPrediction, err error) {

	deviceID, errI := d.GetID("devices", device)
	if errI != nil {
		return
	}
	...
	err = json.Unmarshal([]byte(result2), &lastlocdata)
	if err != nil {
		return
	}
	...
	err = json.Unmarshal([]byte(p), &aidata)
	if err != nil {
		return
	}
