...
import (
	...
	"github.com/wryd-live/wryd-learn-server/server/main/src/learning/nb1"
	"github.com/wryd-live/wryd-learn-server/server/main/src/models"
	"github.com/wryd-live/wryd-learn-server/server/main/src/utils"
)
...
func AnalyzeSensorData(s models.SensorData) (aidata models.LocationAnalysis, err error) {
	...
	go func() {
		...
		defer d.Close()
		errInsert := d.AddPrediction(s.Timestamp, s.Device, aidata.Guesses)
		if errInsert != nil {
...
func GetByLocation(family string, minutesAgoInt int, showRandomized bool, activeMinsThreshold int, minScanners int, minProbability float64, deviceCounts map[string]int) (byLocations []models.ByLocation, err error) {
	...
	for _, sensor := range sensors {
		a, errGet := d.GetPrediction(sensor.Timestamp, sensor.Device)
		if errGet != nil {
			continue
		}
